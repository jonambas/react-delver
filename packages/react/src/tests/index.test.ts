...
describe('parse', () => {
  describe('inclusion', () => {
    it('includes sub components', () => {
      const result = parse(['./tests/inclusion.jsx'], { ignoreSubComponents: false });
    ...
    it('ignores sub components', () => {
      const result = parse(['./tests/inclusion.jsx'], { ignoreSubComponents: true });
      expect(result[0].name).toBe('Foo');
      expect(result[0].count).toBe(2);
    ...
    it('includes only specified packages', () => {
      const result = parse(['./tests/inclusion.jsx'], { from: ['package/b'] });
  ...
  describe('prop parser', () => {
    it('finds strings', () => {
      const result = parse(['./tests/props.jsx'], {});
  ...
  describe('raw processing', () => {
    it('bypasses processing', () => {
      const result = parse(['./tests/inclusion.jsx'], { raw: true });
